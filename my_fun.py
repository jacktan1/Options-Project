...
def get_current_price(stock_of_interest, stock_Id, API_key):
    ...
    if (price == None):
        price = pd.read_csv(str('https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=' + stock_of_interest +
...
def historical_final_price(price_history, current_price, days_till_expiry):
    ...
    for n in range(0, len(final_prices)):
        holder = (price_history[n + days_till_expiry] -
                  price_history[n]) / price_history[n]
